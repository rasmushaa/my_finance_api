...
| Purpose | File |
|---------|------|
| Actual BigQuery schema definitions | `scripts/init_gbq_tables.py` |
| DI container & service wiring | `app/core/container.py` |
| Auth schemas & JWT logic | `app/schemas/auth.py`, `app/services/jwt.py` |
| Exception hierarchy | `app/core/errors/base_error.py` |
| Router registration | `app/main.py`, `app/api/v1.py` |
...
- **Exceptions**: raise only subclasses of `AppError` (from `app/core/errors/base_error.py`). The global `app_error_handler` converts them to HTTP responses automatically.
...
- When writing tests, mirror the structure of existing test files (e.g. `tests/unit/test_api_auth.py`): use `pytest`, mock via DI container overrides
